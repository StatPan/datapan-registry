...
- Registered external adapters: `airport`, `andong`, `calspia`, `car`, `car365`, `codil`, `consumer`, `culture`, `data-gg`,
  ...
  `wamis`, `work`, `work24`, `worldjob`
- External adapter coverage: `5065` registered-adapter operations out of `5075`
  external endpoint operations (`99.4%`)
- Missing external adapter hosts: `11`
- Provider split readiness: `ready`
  (`74` adapters, `74` verification-capable, `23` call-capable)
- Runtime verification evidence: `1716` bounded checks merged into
  `reports/latest-verification.json` (`378` verified, `145` failed, `1193`
  skipped)
- Runtime evidence growth target: `10.3%` checked evidence is above the
  ...
  target.
- Institution API overview: `411` organizations, `12060` APIs, and `16685`
