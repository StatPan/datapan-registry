...
- External adapter coverage: `9242` registered-adapter operations out of `9252`
  external endpoint operations (`99.7%`)
- Missing external adapter hosts: `11`
- Provider split readiness: `ready`
  (`122` adapters, `122` verification-capable, `23` call-capable)
- Runtime verification evidence: `4379` bounded checks merged into
  `reports/latest-verification.json` (`2504` verified, `332` failed, `1543`
  skipped)
- Runtime evidence growth target: `21.0%` checked evidence is above the
  unrounded `10%` release target; `0` additional records are required for this
  target.
- Institution API overview: `411` organizations, `12060` APIs, and `20862`
