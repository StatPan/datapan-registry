...
- Registered external adapters: `airport`, `andong`, `calspia`, `car`, `car365`, `codil`, `culture`, `data-gg`,
  ...
  `wamis`, `work`, `work24`
- External adapter coverage: `4265` registered-adapter operations out of `4275`
  external endpoint operations (`99.3%`)
- Missing external adapter hosts: `11`
- Provider split readiness: `ready`
  (`67` adapters, `67` verification-capable, `23` call-capable)
- Runtime verification evidence: `1589` bounded checks merged into
  `reports/latest-verification.json` (`311` verified, `115` failed, `1163`
  skipped)
- Runtime evidence growth target: `10.0%` checked evidence is above the
  ...
  target.
- Institution API overview: `411` organizations, `12060` APIs, and `15885`
