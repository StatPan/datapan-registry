...
- Runtime verification evidence: `499` bounded checks merged into
  `reports/latest-verification.json` (`22` verified, `87` failed, `390`
  skipped)
- Runtime evidence growth target: `4.1%` checked evidence against the `10%`
  release target; `722` additional records are still required.
