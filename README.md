...
- Registered external adapters: `airport`, `andong`, `culture`, `data-gg`,
  ...
  `wamis`, `work`, `work24`
- External adapter coverage: `3784` registered-adapter operations out of `3794`
  external endpoint operations (`99.2%`)
- Missing external adapter hosts: `11`
- Provider split readiness: `ready`
  (`60` adapters, `60` verification-capable, `23` call-capable)
- Runtime verification evidence: `1541` bounded checks merged into
  `reports/latest-verification.json` (`265` verified, `113` failed, `1163`
  skipped)
- Runtime evidence growth target: `10.0%` checked evidence is above the
  ...
  target.
- Institution API overview: `411` organizations, `12060` APIs, and `15404`
