...
DEFAULT_RECEIPT_GLOB = ".datapan/runtime-evidence/*-credentialed-receipt.json"
...
def validate_receipt(receipt: dict[str, Any], schema: dict[str, Any], policy: dict[str, Any], label: str) -> None:
...
def run_self_tests(schema: dict[str, Any], policy: dict[str, Any]) -> None:
    ...
    validate_receipt(sample, schema, policy, "<self-test-valid>")
    invalid_key = dict(sample)
    invalid_key["credential_value"] = "abc123"
    try:
        validate_receipt(invalid_key, schema, policy, "<self-test-forbidden-key>")
    except ValueError:
        pass
    else:
    ...
    try:
        validate_receipt(invalid_pattern, schema, policy, "<self-test-forbidden-pattern>")
    except ValueError:
        pass
    else:
...
def main() -> int:
    ...
    try:
        ...
        for receipt_path in receipts:
            validate_receipt(load_json(receipt_path), schema, policy, receipt_path.as_posix())
    ...
    print(f"ok credential runtime receipts (receipts={len(receipts)}, self_tests=3)")
    return 0
