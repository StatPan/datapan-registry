...
`fixtures/health-probe-catalog/cli-health-probe-v1.json` proves that the two representative selectors resolve to the gateway and registered external-adapter contracts used by datapan-cli. `scripts/validate-health-probe-catalog.py` checks Registry resolution, CLI operation-key compatibility, bounded parameters, dependency classification, fixture coverage, and release-manifest digests.
