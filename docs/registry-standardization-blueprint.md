...
- Runtime evidence coverage is much lower than callable coverage. Gira #19,
  ...
  Gira #41, and Gira #43 continue gateway boundary evidence growth to `656`,
  but the release readiness warning remains active: the `10%` target requires
  `1,221` evidence records, so `565` additional records are still required.
...
14. Expand runtime verification evidence by source/provider priority. Started
    ...
    then by Gira #39, Gira #41, and Gira #43 with the next gateway batches;
