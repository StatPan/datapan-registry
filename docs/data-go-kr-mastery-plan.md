...
`대전교통공사`, `국가철도공단`, `대전광역시 유성구`,
`한국체육산업개발주식회사`, `한국연구재단`,
`한국과학기술정보연구원`, and `한국산업기술기획평가원`, now
...
11. Execute the next runtime verification batches for gateway and registered
   ...
   `12,060` APIs (`87.3%`).
   The 한국수자원공사 cleanup registers GIMS adapter coverage, materializes the
   remaining `5` APIs and `5` operations, and merges `5` bounded checks (`5`
   verified, `0` failed, `0` skipped). That brings checked runtime evidence to
   ...
   (`88.3%`).
   The 서울특별시 batch registers Seoul TData, Seoul Map, Jongno, and KOSMES
   host coverage, materializes `68` APIs and `136` operations, and merges `100`
   bounded checks (`93` verified, `1` failed, `6` skipped). That brings checked
   ...
   The 한국환경공단 batch registers Recycling Info host coverage, materializes
   `1` API and `1` operation, and merges `1` verified bounded check. That
   ...
   The 충청북도 batch registers Chungbuk Tour host coverage, materializes `1`
   ...
   merges `95` bounded checks (`70` verified and `25` skipped for Seoul Open
   Data auth). That brings checked runtime evidence to `3622`, verified checks
   to `1805`, registered external adapter operations to `8516`, and raises API
   operation coverage to `11,044` of `12,060` APIs (`91.6%`).
   ...
   checked runtime evidence to `3625`, verified checks to `1808`, registered
   external adapter operations to `8519`, and raises API operation coverage to
   `11,046` of `12,060` APIs (`91.6%`).
   ...
   verified and `28` existing EPost boundary skips). That brings checked runtime
   ...
   checked runtime evidence to `4113`, verified checks to `2264`, registered
   external adapter operations to `8963`, and raises API operation coverage to
   `11,235` of `12,060` APIs (`93.2%`).
   ...
   (`2` verified and `26` failed request-boundary records). That brings checked
   ...
   verified NRF bounded checks. That brings checked runtime evidence to `4304`,
   verified checks to `2429`, registered external adapter operations to `9167`,
   and raises API operation coverage to `11,338` of `12,060` APIs (`94.0%`).
   The 한국과학기술정보연구원 batch adds KISTI host coverage for
   `scienceon.kisti.re.kr`, `www.ntis.go.kr`, and `aida.kisti.re.kr`,
   materializes `25` APIs and `27` operations, and merges `27` verified KISTI
   bounded checks. That brings checked runtime evidence to `4331`, verified
   checks to `2456`, registered external adapter operations to `9194`, and
   raises API operation coverage to `11,363` of `12,060` APIs (`94.2%`).
   The 한국산업기술기획평가원 batch adds KEIT host coverage for
   `www.nabis.go.kr` and `www.sobujang.net`, materializes `25` APIs and `48`
   operations, and merges `48` verified KEIT bounded checks. That brings
   ...
   and merges `3` verified bounded checks. That brings checked runtime evidence
   to `4467`, verified checks to `2590`, registered external adapter operations
   to `9330`, and raises API operation coverage to `11,435` of `12,060` APIs
   (`94.8%`).
   The 국가유산청 국립고궁박물관 batch adds Gogung host coverage for
   `www.gogung.go.kr`, materializes the remaining `22` APIs and `22`
   operations, and merges `22` verified bounded checks. That brings checked
   runtime evidence to `4489`, verified checks to `2612`, registered external
   adapter operations to `9352`, and raises API operation coverage to `11,457`
   of `12,060` APIs (`95.0%`).
   ...
   operations, and merges `31` verified bounded checks. That brings checked
   runtime evidence to `4639`, verified checks to `2729`, registered external
   adapter operations to `9502`, and raises API operation coverage to `11,511`
   of `12,060` APIs (`95.4%`).
   ...
   merges `20` verified bounded checks. That brings checked runtime evidence to
   ...
   failed on `mnd-open-data_request_failed`). That brings checked runtime
   evidence to `4728`, verified checks to `2808`, failed checks to `377`,
   registered external adapter operations to `9591`, and raises API operation
   coverage to `11,553` of `12,060` APIs (`95.8%`).
   The 한국은행 batch adds ECOS host coverage for `ecos.bok.or.kr`,
   materializes `16` APIs and `16` operations, and merges `16` verified bounded
   ...
   The 한국수목원정원관리원 batch adds KOAGI SeedPedia host coverage for
   `seedpedia.koagi.or.kr`, materializes `6` APIs and `8` operations, and
   merges `8` verified bounded checks. That brings checked runtime evidence to
