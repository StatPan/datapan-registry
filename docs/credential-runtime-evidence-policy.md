...
Reviewed checked-in receipts live under `reports/credential-runtime-receipts/`. Default secret-free CI validates that reviewed intake path with `python3 scripts/validate-credential-runtime-receipts.py`; checked-in receipts require `review` metadata with an allowed review state. A reviewed receipt can become compatibility-relief eligible only when it is present, schema-valid, redaction-safe, reviewed, and accepted for relief.
